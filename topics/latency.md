...
I said that Valkey is *mostly* single threaded.
We do use threads in Valkey in order to perform I/O operations (including command parsing and I/O operations)
in the background, but this does not change the fact
that Valkey executes all the commands sequentially using a single thread.

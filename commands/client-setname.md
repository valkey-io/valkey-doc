...
There is no limit to the length of the name that can be assigned if not the usual limits of the Valkey string type (512 MB). However it is not possible to use spaces in the connection name as this would violate the format of the `CLIENT LIST` reply.

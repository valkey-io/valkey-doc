...
Since Valkey 2.2 many data types are optimized to use less space up to a certain size.
...
using the following server.conf directives (defaults are shown):
...
Valkey 2.2 introduced new bit and byte level operations: `GETRANGE`, `SETRANGE`, `GETBIT` and `SETBIT`.
Using these commands you can treat the Valkey string type as a random access array.
...
it contains grows too large (you can configure the limit in server.conf).
...
This is the result against a 64 bit instance of Valkey 2.2:
...
*WARNING*: for this to work, make sure that in your server.conf you have
something like this:

...
* [Create a Valkey Cluster](#create-a-redis-cluster)
...
* [Upgrade nodes in a Valkey Cluster](#upgrade-nodes-in-a-redis-cluster)
* [Migrate to Valkey Cluster](#migrate-to-redis-cluster)
...
At minimum, set the following directives in the `server.conf` file:
...
Create a `server.conf` file inside each of the directories, from 7000 to 7005.
...
You can also test your Valkey Cluster using the `redis-cli` command line utility:
...
* Create a server.conf file inside, similar to the one used for the other nodes but using 7006 as port number.
* Finally start the server with `../redis-server ./server.conf`
...
* There is a configuration parameter that controls the replica migration feature that is called `cluster-migration-barrier`: you can read more about it in the example `server.conf` file provided with Valkey Cluster.

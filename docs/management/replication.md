...
`server.conf` shipped with the Valkey distribution for more information.
...
parameter. Please refer to the example `server.conf` file in the Valkey distribution
for more details.

## Read-only replica

Since Valkey 2.6, replicas support a read-only mode that is enabled by default.
This behavior is controlled by the `replica-read-only` option in the server.conf file, and can be enabled and disabled at runtime using `CONFIG SET`.
...
For more information, please check the example `server.conf` file shipped with the
...
into `server.conf`, that may be different from the forwarded port in case
...
And are documented in the example `server.conf` of recent Valkey distributions.

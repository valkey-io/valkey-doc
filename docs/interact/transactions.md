...
Starting with Valkey 2.6.5, the server will detect an error during the accumulation of commands.
It will then refuse to execute the transaction returning an error during `EXEC`, discarding the transaction.

> **Note for Valkey < 2.6.5:** Prior to Valkey 2.6.5 clients needed to detect errors occurring prior to `EXEC` by checking
...
**NOTE**
* In Valkey versions before 6.0.9, an expired key would not cause a transaction

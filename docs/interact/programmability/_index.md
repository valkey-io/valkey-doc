...
Valkey provides a programming interface that lets you execute custom scripts on the server itself. In Valkey 7 and beyond, you can use [Valkey Functions](/docs/manual/programmability/functions-intro) to manage and run your scripts. In Valkey 6.2 and below, you use [Lua scripting with the EVAL command](/docs/manual/programmability/eval-intro) to program the server.
...
Firstly, and ever since Valkey 2.6.0, the `EVAL` command enables running server-side scripts.
...
Read-only scripts and read-only script commands were introduced in Valkey 7.0

* Before Valkey 7.0.1 `PUBLISH`, `SPUBLISH` and `PFCOUNT` were not considered write commands in scripts
* Before Valkey 7.0.1 the `no-writes` [flag](/topics/lua-api#script_flags) did not imply `allow-oom`
* Before Valkey 7.0.1 the `no-writes` flag did not permit the script to run during write pauses.
...
either via `server.conf` or by using the `CONFIG SET` command.

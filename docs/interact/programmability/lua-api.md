...
```lua
> EVAL "return server.call('ECHO', 'Echo,', 'echo... ', 'eco... ', 'o...')" 0
...
```
> EVAL "return { err = 'ERR My very special table error' }" 0
(error) ERR My very special table error
> EVAL "return server.error_reply('ERR My very special reply error')" 0
...
```
> EVAL "return { ok = 'TICK' }" 0
TICK
> EVAL "return server.status_reply('TOCK')" 0
TOCK
```
...
As of Valkey version 2.6.0, scripts were replicated verbatim, meaning that the scripts' source code was sent for execution by replicas and stored in the AOF.
An alternative replication mode added in version 3.2.0 allows replicating only the scripts' effects.
As of Valkey version 7.0, script replication is no longer supported, and the only replication mode available is script effects replication.
...
```lua
server.replicate_commands() -- Enable effects replication in versions lower than Valkey v7.0
...
**Note:**
as of Valkey v7.0, verbatim script replication is no longer supported.
...
> EVAL "return server.call('get','foo')" 0
"bar"
```
...
It is available as an opt-in choice as of Valkey v6.0.
...
**Note:**
the RESP3 [big number](https://github.com/redis/redis-specifications/blob/master/protocol/RESP3.md#big-number-type) and [verbatim strings](https://github.com/redis/redis-specifications/blob/master/protocol/RESP3.md#verbatim-string-type) replies are only supported as of Valkey v7.0 and greater. 
...
However, one subtle aspect of the behavior changed between Valkey versions 2.8.11 and 2.8.12.
Prior to Valkey version 2.8.12, the database selected by the Lua script was *set as the current database* for the client connection that had called it.
As of Valkey version 2.8.12, the database selected by the Lua script only affects the execution context of the script, and does not modify the database that's selected by the client calling the script.
...
```
> EVAL "return { struct.unpack('HH', ARGV[1]) }" 0 "\x01\x00\x02\x00"
1) (integer) 1
2) (integer) 2
3) (integer) 5
```
...
```
> EVAL "return cmsgpack.unpack(ARGV[1])" 0 "\x93\xa3foo\xa3bar\xa3baz"
1) "foo"
2) "bar"
3) "baz"
```
...
```
> EVAL 'return bit.tohex(422342)' 0
"000671c6"
```
...
```
> EVAL 'return bit.bor(1,2,4,8,16,32,64,128)' 0
(integer) 255
```

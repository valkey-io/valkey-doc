...
set the configuration directive using the `server.conf` file, or later using
the `CONFIG SET` command at runtime.

For example, to configure a memory limit of 100 megabytes, you can use the
following directive inside the `server.conf` file:
...
However, since Valkey 3.0 the algorithm was improved to also take a pool of good
...
As you can see Valkey 3.0 does a better job with 5 samples compared to Valkey 2.8, however most objects that are among the latest accessed are still retained by Valkey 2.8. Using a sample size of 10 in Valkey 3.0 the approximation is very close to the theoretical performance of Valkey 3.0.
...
Starting with Valkey 4.0, the [Least Frequently Used eviction mode](http://antirez.com/news/109) is available. This mode may work better (provide a better
...
Instructions about how to tune these parameters can be found inside the example `server.conf` file in the source distribution. Briefly, they are:
...
So basically the factor is a trade off between better distinguishing items with low accesses VS distinguishing items with high accesses. More information is available in the example `server.conf` file.

...
`initServer` function defined in `server.c` initializes the numerous fields of the `redisServer` structure variable. One such field is the Valkey event loop `el`:
...
`aeCreateTimeEvent` accepts the following as parameters:
  ...
  * `eventLoop`: This is `server.el` in `server.c`
...
`initServer` calls `aeCreateTimeEvent` to add a timed event to `timeEventHead` field of `server.el`. `timeEventHead` is a pointer to a list of such timed events. The call to `aeCreateTimeEvent` from `server.c:initServer` function is given below:

    aeCreateTimeEvent(server.el /*eventLoop*/, 1 /*milliseconds*/, serverCron /*proc*/, NULL /*clientData*/, NULL /*finalizerProc*/);

`server.c:serverCron` performs many operations that helps keep Valkey running properly.
...
Following is an explanation of what precisely `aeCreateFileEvent` does when called from `server.c:initServer`.
...
`ae.c:aeMain` called from `server.c:main` does the job of processing the event loop that is initialized in the previous phase.
...
Now, `aeProcessEvents` calls the `server.c:acceptHandler` registered as the callback. `acceptHandler` executes [accept](http://man.cx/accept) on the _listening descriptor_ returning a _connected descriptor_ with the client. `server.c:createClient` adds a file event on the _connected descriptor_ through a call to `ae.c:aeCreateFileEvent` like below:

...
If the `ROLE` commands is not available (it was introduced in Valkey 2.8.12), a client may resort to the `INFO replication` command parsing the `role:` field of the output.
...
Starting with Valkey 2.8.12, when Valkey Sentinel changes the configuration of

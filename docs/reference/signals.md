...
The information in this document **only applies to Valkey version 2.6 or greater**.
...
Since Valkey 2.6.11, no further attempt to shut down will be made unless a new `SIGTERM` is received or the `SHUTDOWN` command is issued.

Since Valkey 7.0, the server waits for lagging replicas up to a configurable `shutdown-timeout`, 10 seconds by default, before shutting down.
...
* Since Valkey 2.8, a fast memory test is performed as a first check of the reliability of the crashing system.
...
generating an error. Since Valkey version 2.6.10, this can be done using the signal `SIGUSR1`. This signal is handled in a special way:

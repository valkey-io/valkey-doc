...
The information contained in this document is **only applicable to Valkey version 2.6 or greater**.
...
In Valkey 2.4 there was a hard-coded limit for the maximum number of clients
that could be handled simultaneously.

In Valkey 2.6 and newer, this limit is configurable using the `maxclients` directive in `server.conf`. The default is 10,000 clients.
...
more and more memory. For this reason by default Valkey sets limits to the
...
It is possible to change the limit at runtime using the `CONFIG SET` command or in a permanent way using the Valkey configuration file `server.conf`. See the example `server.conf` in the Valkey distribution for more information about how to set the limit.
...
`maxmemory-clients` can be set permanently in the configuration file (`server.conf`) or via the `CONFIG SET` command.
...
For more information and an example refer to the `maxmemory-clients` section in the default `server.conf` file.

Client eviction is available from Valkey 7.0.
...
You can configure this limit via `server.conf` or simply using `CONFIG SET timeout <value>`.
...
* **flags**: The kind of client (N means normal client, check the [full list of flags](https://server.io/commands/client-list)).
* **omem**: The amount of memory used by the client for the output buffer.
* **cmd**: The last executed command.

See the [`CLIENT LIST`](https://server.io/commands/client-list) documentation for the full listing of fields and their purpose.
...
The commands `CLIENT SETNAME` and `CLIENT GETNAME` can be used to set and get the connection name. Starting with Valkey 4.0, the client name is shown in the
